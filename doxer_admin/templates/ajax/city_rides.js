            ...
            success: function (data) {
                if ($(this).hasClass('active')){
                    ...
                    $.each(data.results, function(i, val) {
                        ...
                        $('#tests-table').append("<tr class='"+ tr +"'><td> # " + val.id  + "</td><td>" + val.trip_date + "<br><br>"+val.ride_time+"</td><td>"+ val.create_at +"</td><td>" + val.getdr + "</td><td>"+ val.getpas +"</td><td>"+ val.vehicle +"</td><td>" + val.Location + "<br><hr/>" + val.destination + "</td><td><center>"+ val.status +"</center></td><th>  "+ val.fees + "</th><td><center><button class='btn btn-success btn-rounded btn-sm' id='pleaseclickme' data='"+val.rid+"'><i class='icon-direction btn-icon-append'></i></button></td></tr>")
                        });
...
$(document).on('click','#pleaseclickme',function(){
   ...
   $.ajax({
        type: "POST",
        url: "{% url 'doxer_admin:Incitymap_view' %}", 
